...
#include<stdlib.h>
#include<string.h>
#include <unistd.h> // Included for sleep command
...
int main(int argc, char* argv[]){
   ...
   else{ // If argument does not match previous if statements.
	    printf("Invalid command!\n"); // Prints invalid command.
      printf("Flashing LED!\n"); // Prints Flashing LED
      removeTrigger(); // Removes trigger to prepare for LED config
      while (1) { // Endlessly flashes LED in 1 second on, 1 second off fashion
        writeLED("/brightness", "1");
        sleep(1);
        writeLED("/brightness", "0");
        sleep (1);
